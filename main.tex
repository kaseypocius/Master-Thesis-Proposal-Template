...
% Comment out the following line to compile without biblatex
\def\withbiblatex{} % clear cache before commenting this off
...
% Programatically clean up the bibliography here, rather than manually editing the BibTeX file
\AtEveryBibitem{%
\clearfield{doi}
\clearfield{pages}
  \ifentrytype{inproceedings}{%true
    \clearlist{address}
    \clearfield{isbn}
    \clearlist{location}
    \clearfield{month}
...
\renewcommand{\cite}{\parencite} % format citations between parentheses
\else 
\usepackage{apacite}
\fi
